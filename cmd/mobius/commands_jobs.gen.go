...
func registerJobsCommands(app *cli.App) {
	...
	jobsGrp.Command("claim").
		...
		Flags(
			...
			cli.Int("concurrency-limit", "").Help("Maximum number of concurrent in-flight jobs this worker process intends to hold. Recorded on the wo…"),
			...
			cli.String("worker-id", "").Help("Deprecated alias for `worker_instance_id`. Accepted for one release while customer SDKs migrate. Ne…"),
			cli.String("worker-instance-id", "").Help("Caller-configured stable identifier for the worker process. The SDK auto-detects this on Cloud Run,…"),
			cli.String("worker-name", "").Help("Optional human-readable name shown in the worker list."),
			cli.String("worker-session-token", "").Help("Per-boot token generated by the SDK on Worker construction. Stamped onto the claimed job and re-pre…"),
		...
		Run(func(ctx *cli.Context) error {
			...
			if ctx.IsSet("concurrency-limit") {
				v := ctx.Int("concurrency-limit")
				body.ConcurrencyLimit = &v
			}
			if ctx.IsSet("queues") {
			...
			if ctx.String("file") == "" && !ctx.IsSet("actions") && !ctx.IsSet("agent-id") && !ctx.IsSet("agent-session-id") && !ctx.IsSet("concurrency-limit") && !ctx.IsSet("queues") && !ctx.IsSet("wait-seconds") && !ctx.IsSet("worker-id") && !ctx.IsSet("worker-instance-id") && !ctx.IsSet("worker-name") && !ctx.IsSet("worker-session-token") && !ctx.IsSet("worker-version") {
	...
	jobsGrp.Command("complete-job").
		...
		Flags(
			...
			cli.String("worker-id", "").Help("Deprecated alias for `worker_instance_id`. Used as the fence only when `worker_session_token` is ab…"),
			cli.String("worker-instance-id", "").Help("Echo of the claim's `worker_instance_id`. Audit only."),
			cli.String("worker-session-token", "").Help("Per-boot token from the claim. Compared against the job's `claimed_by_session_token`; mismatch retu…"),
			cli.String("file", "f").Help("Request body from a file (JSON or YAML, '-' for stdin). Flags override file contents."),
			cli.Bool("dry-run", "").Help("Print the assembled request body and exit without sending it."),
		).
		Use(cli.RequireFlags("api-key")).
	...
	jobsGrp.Command("emit-job-events").
		...
		Flags(
			...
			cli.String("worker-id", "").Help("Deprecated alias for `worker_instance_id`. Used as the fence only when `worker_session_token` is ab…"),
			cli.String("worker-instance-id", "").Help("Echo of the claim's `worker_instance_id`. Audit only."),
			cli.String("worker-session-token", "").Help("Per-boot token from the claim. Used as the lease fence in preference to `worker_id`."),
			cli.String("file", "f").Help("Request body from a file (JSON or YAML, '-' for stdin). Flags override file contents."),
			cli.Bool("dry-run", "").Help("Print the assembled request body and exit without sending it."),
		).
		Use(cli.RequireFlags("api-key")).
	...
	jobsGrp.Command("heartbeat").
		...
		Flags(
			cli.Int("attempt", "").Help("[required] Must match the `attempt` from the original claim."),
			cli.String("worker-id", "").Help("Deprecated alias for `worker_instance_id`. Used as the lease fence only when `worker_session_token`…"),
			cli.String("worker-instance-id", "").Help("Echo of the `worker_instance_id` from the claim. Logged for audit; not the fence value."),
			cli.String("worker-session-token", "").Help("Per-boot token from the original claim. Compared against the job's `claimed_by_session_token`; mism…"),
			cli.String("file", "f").Help("Request body from a file (JSON or YAML, '-' for stdin). Flags override file contents."),
			cli.Bool("dry-run", "").Help("Print the assembled request body and exit without sending it."),
		).
		Use(cli.RequireFlags("api-key")).
