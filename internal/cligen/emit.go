...
func isReservedFlag(name string) bool {
	switch name {
	case "api-url", "api-key", "project", "log-level", "file":
		return true
	}
	return false
}
...
func renderCommand(b *bytes.Buffer, group string, c PlannedCommand) error {
	...
	if c.Body != nil {
		for _, f := range c.Body.Fields {
			...
			case "strings":
				flags = append(flags, fmt.Sprintf(`cli.Strings(%q, "").Help(%q)`, f.FlagName, help))
			case "json":
				flags = append(flags, fmt.Sprintf(`cli.String(%q, "").Help(%q)`, f.FlagName, help+" (JSON)"))
			}
		}
		flags = append(flags, `cli.String("file", "f").Help("Request body as JSON (path to file, or '-' for stdin). Flags override file contents.")`)
	}
	if len(flags) > 0 {
		fmt.Fprintf(b, "\t\tFlags(\n")
		for _, f := range flags {
	...
	if c.Body != nil {
		...
		for _, f := range c.Body.Fields {
			...
			case "json":
				fmt.Fprintf(b, "\t\t\tif ctx.IsSet(%q) { if err := json.Unmarshal([]byte(ctx.String(%q)), &body.%s); err != nil { return fmt.Errorf(\"--%s: invalid JSON: %%w\", err) } }\n",
					f.FlagName, f.FlagName, f.GoField, f.FlagName)
			}
		}
		...
		if !hasRequired {
			...
			for _, f := range c.Body.Fields {
				...
				conds = append(conds, fmt.Sprintf("!ctx.IsSet(%q)", f.FlagName))
			}
