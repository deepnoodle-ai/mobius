...
export interface paths {
    ...
    "/v1/projects/{project}/channels/{id}/messages": {
         ...
         * @description Posts a message to the channel. For agent-sent messages, supply `sender_agent_id` (and optionally `sender_session_id`) to attribute the message to the agent rather than the authenticated user. Content is Markdown. Use `reply_to` for threading.
         */
...
export interface components {
    schemas: {
        ...
        /** @description Fields used to post a new channel message. */
        ...
        /**
         * @description Body for `POST /jobs/claim`. Workers must send either `worker_instance_id` (preferred) or `worker_id` (deprecated alias for one release). Sending neither is a 400; sending both with conflicting values is a 400.
         *
        ...
        /** @description Lease fence presented on heartbeat. Either `worker_session_token` (preferred) or `worker_id` (deprecated alias) must be supplied along with `attempt`. The at-least-one constraint on the worker identifier pair is enforced server-side; only `attempt` is marked required at the schema level. */
        JobFenceRequest: {
            /**
             * @deprecated
        ...
        /** @description Terminal report for a claimed job. Either `worker_session_token` (preferred) or `worker_id` (deprecated) must be supplied along with `attempt` and `status`. The at-least-one constraint on the worker identifier pair is enforced server-side. */
        JobCompleteRequest: {
            /**
             * @deprecated
        ...
        /** @description Final response that completed an interaction. */
        InteractionResponse: {
        ...
        /** @description One member response captured while a group interaction waits for all members. */
