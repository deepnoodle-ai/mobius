...
// JobClaimRequest Body for `POST /jobs/claim`. Workers must send either `worker_instance_id` (preferred) or `worker_id` (deprecated alias for one release). Sending neither is a 400; sending both with conflicting values is a 400.
//
...
type JobClaimRequest struct {
	...
	// WorkerId Deprecated alias for `worker_instance_id`. Accepted for one release while customer SDKs migrate. New integrations must send `worker_instance_id` instead.
	// Deprecated: this property has been marked as deprecated upstream, but no `x-deprecated-reason` was set
	WorkerId *string `json:"worker_id,omitempty"`

	// WorkerInstanceId Caller-configured stable identifier for the worker process. The SDK auto-detects this on Cloud Run, Kubernetes, Fly, Railway and Render; otherwise a UUID is generated at boot. Stored on the claimed job as `claimed_by` for human- readable display.
	...
	// WorkerSessionToken Per-boot token generated by the SDK on Worker construction. Stamped onto the claimed job and re-presented as the lease fence on heartbeat / events / complete. Opaque to operators. Required for the new fence; absence falls back to the legacy `worker_id`-based fence for one release.
...
// JobCompleteRequest Terminal report for a claimed job. Either `worker_session_token` (preferred) or `worker_id` (deprecated) must be supplied along with `attempt` and `status`. The at-least-one constraint on the worker identifier pair is enforced server-side.
type JobCompleteRequest struct {
	...
	// WorkerId Deprecated alias for `worker_instance_id`. Used as the fence only when `worker_session_token` is absent.
	// Deprecated: this property has been marked as deprecated upstream, but no `x-deprecated-reason` was set
	WorkerId *string `json:"worker_id,omitempty"`
...
type JobEventsRequest struct {
	...
	// WorkerSessionToken Per-boot token from the claim. Used as the lease fence in preference to `worker_id`.
	WorkerSessionToken *string `json:"worker_session_token,omitempty"`
}

// JobFenceRequest Lease fence presented on heartbeat. Either `worker_session_token` (preferred) or `worker_id` (deprecated alias) must be supplied along with `attempt`. The at-least-one constraint on the worker identifier pair is enforced server-side; only `attempt` is marked required at the schema level.
type JobFenceRequest struct {
	// Attempt Must match the `attempt` from the original claim.
	Attempt int `json:"attempt"`

	// WorkerId Deprecated alias for `worker_instance_id`. Used as the lease fence only when `worker_session_token` is absent.
	// Deprecated: this property has been marked as deprecated upstream, but no `x-deprecated-reason` was set
	WorkerId *string `json:"worker_id,omitempty"`
	...
	// WorkerSessionToken Per-boot token from the original claim. Compared against the job's `claimed_by_session_token`; mismatch returns 409 lease-lost. Absence falls back to legacy `worker_id` fence.
	WorkerSessionToken *string `json:"worker_session_token,omitempty"`
}
