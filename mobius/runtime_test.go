...
func TestRuntimeClaim_Job(t *testing.T) {
	claimBody := `{"job_id":"job_1","run_id":"run_1","workflow_name":"hello","step_name":"greet","spec":{"kind":"action","name":"print","parameters":{"msg":"hi"}},"attempt_number":1,"queue":"default","lease_token":"lease-1","heartbeat_interval_seconds":15}`
...
func TestRuntimeClaim_EscapesProjectHandle(t *testing.T) {
	...
	claimBody := `{"job_id":"job_1","run_id":"run_1","workflow_name":"hello","step_name":"greet","spec":{"kind":"action","name":"print","parameters":{}},"attempt_number":1,"queue":"default","lease_token":"lease-1","heartbeat_interval_seconds":15}`
...
func TestRuntimeCompleteSuccess(t *testing.T) {
	...
	c, _ := newTestClient(t, h)
	job := &runtimeJob{JobID: "job_1", Attempt: 1, WorkerInstanceID: "w1", LeaseToken: "lease-1"}
...
func TestRuntimeCompleteFailure_LeaseLost(t *testing.T) {
	...
	c, _ := newTestClient(t, h)
	err := c.runtimeCompleteFailure(context.Background(), &runtimeJob{JobID: "job_1", Attempt: 1, WorkerInstanceID: "w1", LeaseToken: "lease-1"}, "Error", "boom")
	assert.ErrorIs(t, err, ErrLeaseLost)
}
...
func TestRuntimeCompleteFailure_Body(t *testing.T) {
	...
	c, _ := newTestClient(t, h)
	err := c.runtimeCompleteFailure(context.Background(), &runtimeJob{JobID: "job_1", Attempt: 2, WorkerInstanceID: "w1", LeaseToken: "lease-1"}, "Timeout", "deadline exceeded")
	assert.NoError(t, err)
...
func TestWorkerRun_ClaimsNextJobOnlyAfterCurrentCompletes(t *testing.T) {
	...
	h := http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case r.URL.Path == "/v1/projects/test-project/jobs/claim":
			...
			if n == 1 {
				w.Header().Set("Content-Type", "application/json")
				_, _ = io.WriteString(w, `{"job_id":"job_1","run_id":"run_1","workflow_name":"hello","step_name":"greet","spec":{"kind":"action","name":"block","parameters":{}},"attempt_number":1,"queue":"default","lease_token":"lease-1","heartbeat_interval_seconds":3600}`)
				return
			}
			select {
...
func TestWorkerPool_RunUsesDistinctSingleJobWorkers(t *testing.T) {
	...
	h := http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case r.URL.Path == "/v1/projects/test-project/jobs/claim":
			...
			if n <= 3 {
				w.Header().Set("Content-Type", "application/json")
				_, _ = io.WriteString(w, fmt.Sprintf(`{"job_id":"job_%d","run_id":"run_1","workflow_name":"hello","step_name":"greet","spec":{"kind":"action","name":"block","parameters":{}},"attempt_number":1,"queue":"default","lease_token":"lease-%d","heartbeat_interval_seconds":3600}`, n, n))
				return
			}
			w.WriteHeader(http.StatusNoContent)
